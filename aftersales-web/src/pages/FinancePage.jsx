...
// 🔴 [แก้ไข] เดิมเช็คแค่ status === "เสร็จสิ้น"/"เสร็จแล้ว" (ต้องรอช่างส่งรายงาน
// ปิดงานก่อนเท่านั้นถึงจะออกบิลได้) แต่จริง ๆ แล้วราคา (estimated_price) กับ
// คำขอเบิกอะไหล่ถูกบันทึก/อนุมัติเสร็จได้ตั้งแต่ตอนช่างยังซ่อมอยู่ (สถานะ
// "กำลังซ่อม") ก่อนส่งรายงานปิดงานจริงด้วยซ้ำ (ดู job_detail.dart ฝั่งช่าง —
// ปุ่ม "บันทึกราคาประเมิน" อยู่คนละจุดกับปุ่มส่งรายงานปิดงาน) แอดมินเลยต้องรอ
// เฉย ๆ ทั้งที่ข้อมูลพร้อมออกบิลแล้ว — เปลี่ยนเงื่อนไขเป็น "ช่างบันทึกราคาไว้แล้ว
// (estimated_price > 0) + คำขอเบิกอะไหล่ของงานนี้ต้องไม่มีรายการค้าง
// "รอดำเนินการ" (ต้องอนุมัติ/ปฏิเสธให้ครบก่อน)" แทน ไม่ต้องรอสถานะ "เสร็จสิ้น"
// อีกต่อไป (ยกเว้นงานที่ยกเลิกแล้ว และงานที่ตั้งยอดไปแล้วซึ่งยังกันซ้ำเหมือนเดิม)
function hasUnresolvedPartRequests(repair, partRequests) {
  if (!repair) return false;
  // 🐛 เทียบ repair_id กับทั้ง record_id และ id เพราะ part_requests บางแถวที่
  // แอปมือถือบันทึกไว้อ้างอิงด้วยคีย์จริงใน Firebase (id) ไม่ใช่ record_id
  // เพียงอย่างเดียว (แพทเทิร์นเดียวกับที่แก้ไว้ใน findRepairInfo() ของ
  // SparePartsPage.jsx) — เทียบแค่แบบเดียวจะพลาดบางแถวไป ทำให้คำขอที่ยังค้าง
  // "รอดำเนินการ" หลุดรอดไม่ถูกนับ
  return (partRequests || []).some(
    (pr) =>
      (String(pr.repair_id) === String(repair.record_id) ||
        String(pr.repair_id) === String(repair.id)) &&
      pr.status === "รอดำเนินการ"
  );
}

export function isInvoiceable(r, partRequests = []) {
  if (!r) return false;
  const status = r.status || "";
  if (status === "ยกเลิกแล้ว") return false;
  const alreadyBilled = Number(r.total_price) > 0;
  if (alreadyBilled) return false;
  const priceRecorded = Number(r.estimated_price) > 0;
  if (!priceRecorded) return false;
  return !hasUnresolvedPartRequests(r, partRequests);
}
...
// 🆕 [ใหม่] initialRepairId — เปิดหน้านี้จากปุ่ม "ออกบิล" ในหน้ารายละเอียดงานซ่อม
// (JobDetailModal) พร้อมเลือกงานนั้นไว้ล่วงหน้าได้เลย เหมือน initialRepairId ที่
// ส่งเข้า AdminCreateInvoicePage ฝั่งแอป (assign_repair_formdetail.dart) —
// ยังปล่อยให้แอดมินเปลี่ยนงานที่เลือกในดรอปดาวน์ได้ตามปกติถ้าต้องการ
export function CreateInvoiceModal({ repairs, partRequests, spareParts, machines, onClose, initialRepairId }) {
  const invoiceableJobs = repairs.filter((r) => isInvoiceable(r, partRequests));
  const [repairId, setRepairId] = useState(initialRepairId != null ? String(initialRepairId) : "");
  ...
  const availablePartRequests = useMemo(() => {
    ...
    return partRequests.filter(
      (pr) => String(pr.repair_id) === String(selectedJob.record_id) && pr.status === "อนุมัติแล้ว"
    );
  ...
  async function handleSave() {
    ...
    try {
      ...
      const updates = {
        ...
        // invoice_date คือฟิลด์ที่แอป Flutter อ่านไปโชว์วันที่ออกบิลในหน้ารายการบิล
        // (แอปเขียนฟิลด์นี้เอง) — เขียนคู่กับ invoiced_at ด้วยค่าเดียวกันเสมอ
        invoice_date: invoicedAt,
        // bill_id คือฟิลด์ที่แอป Flutter (ลูกค้า/ช่าง/แอดมิน) อ่านไปแสดงเลขบิล
        // ต้องเขียนคู่กับ invoice_no เสมอ ไม่งั้นบิลที่ออกจากเว็บจะโชว์ "-" ในแอป
        bill_id: invoiceNo,
  ...
  return (
    ...
    >
      <div>
        ...
        >
          <option value="">เลือกงานที่พร้อมออกบิล...</option>
        ...
        {invoiceableJobs.length === 0 ? (
          <p className="text-xs text-slate-400 mt-2">ไม่มีงานที่พร้อมออกบิลตอนนี้ (ต้องเป็นงานที่ช่างบันทึกราคาไว้แล้ว คำขอเบิกอะไหล่ต้องอนุมัติ/ปฏิเสธครบ และยังไม่เคยออกบิลมาก่อน)</p>
        ) : null}
      </div>
