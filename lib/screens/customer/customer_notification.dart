...
class CustomerNotificationPage extends StatefulWidget {
  ...
  const CustomerNotificationPage({super.key, this.onUnreadCountsChanged});
...
class _CustomerNotificationPageState extends State<CustomerNotificationPage> {
  ...
  /// 💬 เปิดห้องแชทของกลุ่มนี้ + เคลียร์สถานะอ่านแล้วทั้งหมด
  Future<void> _openChatGroup(_ChatGroup group) async {
    ...
    for (final n in group.items) {
      ...
      if (rawId != null) {
        db.DatabaseHelper.instance.markNotificationAsRead(rawId);
      }
    }
  ...
  Future<void> _onTapNotification(Map<String, dynamic> item) async {
    ...
    Future<void> openDetailDestination() async {
      ...
      if (isChat) {
        ...
        await Navigator.of(context).push(
          MaterialPageRoute(
            builder: (context) => CustomerJobDetailPage(repairId: targetId),
          ),
        );
      }
      if (!mounted) return;
